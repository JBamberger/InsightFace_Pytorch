...
if __name__ == '__main__':
    ...
    if args.save:
        video_writer = cv2.VideoWriter(
            conf.data_path/'recording.avi', cv2.VideoWriter_fourcc(*'XVID'), 6, (1280, 720))
    ...
    while cap.isOpened():
        ...
        if isSuccess:
            ...
            try:
                ...
                # shape:[10,4],only keep 10 highest possibility faces
                bboxes = bboxes[:, :-1]
                bboxes = bboxes.astype(int)
                bboxes = bboxes + [-1, -1, 1, 1]  # personal choice
                for idx, bbox in enumerate(bboxes):
                    frame = draw_box_name(bbox, "detection", frame)
                try:
